...
/*   Updated: 2025/10/27 01:11:46 by scambier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_unsigned(t_strb *buffer, t_cs *spec, t_uint32 value, t_uint32 bi);
...
static void	handle_specification(t_strb *buffer, t_conv_spec *spec, va_list ap)
{
	if (spec->specifier == 's')
		...
		add_unsigned(buffer, spec, va_arg(ap, t_uint32), 0);
	else if (spec->specifier == 'x')
		add_unsigned(buffer, spec, va_arg(ap, t_uint32), 1);
	else if (spec->specifier == 'X')
		add_unsigned(buffer, spec, va_arg(ap, t_uint32), 2);
	else if (spec->specifier == 'p')
		add_pointer(buffer, spec, va_arg(ap, t_uint64));
	else
